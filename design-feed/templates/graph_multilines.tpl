<html>
  ...
  <script>
    ...
    var formatDate = d3.time.format("%Y-%m-%d").parse; //%Y-%m-%d %H:%M:%S
    ...
    .rangeRoundBands([0, w], 1).domain(data1.map(function(d) { return d.label; }));
    // Y scale will fit values from 0-10 within pixels h-0 (Note the inverted domain for the y-scale: bigger is up!)
    var y1 = d3.scale.linear().domain([0, d3.max(data1, function(d) { return d.value; })]).range([h, 0]); // in real world the domain would be dynamically calculated from the data
    var y2 = d3.scale.linear().domain([0, d3.max(data2, function(d) { return d.value; })]).range([h, 0]);  // in real world the domain would be dynamically calculated from the data
